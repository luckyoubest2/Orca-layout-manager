...
- 点击「布局」标签，侧边栏内容区显示布局管理器（与官方顶栏「… → Layouts（布局）」
  菜单完全共用同一份数据）：
  - 默认极简列表：仅显示布局名称，点击即应用（与「标签/页面」列表交互一致）；
  - 可选（设置中开启）：「保存新布局」输入框、每行「更新 / 设为默认 / 删除」按钮、
    一键应用默认布局。
...
- 「显示保存新布局入口」：是否显示布局标签顶部的创建/保存布局输入框（默认关闭）。
- 「显示布局行操作按钮」：是否显示每个布局行的保存/默认/删除按钮（默认关闭）；
  关闭后点击布局名称仍可应用布局。
- 「显示前往默认布局的快捷按钮」：在顶栏「前往今日日志」旁新增一个按钮，点击应用
  默认布局（未设置默认布局时仍回到今日日志，默认开启；官方按钮保持不变）。
- 「自动隐藏滚动条」：默认隐藏所有滚动条，仅当鼠标悬停到可滚动区域时显示（默认开启）。
- 「自动隐藏面板操作命令」：默认隐藏每个面板的浮动操作按钮（目录、更多菜单、跳转等），
  鼠标悬停到面板时显示（默认开启）。
- 「隐藏面板分割线」：移除面板之间的纯色分割实线（默认开启）。
- 「卡片化面板」：面板采用统一卡片样式，自带背景色差与柔和投影，圆角与面板间缝隙可调
  （默认开启，圆角 12px、间距 10px，建议圆角 10–12px、间距 8–12px；间距同时作用于
  面板之间与工作区边缘留白）。
